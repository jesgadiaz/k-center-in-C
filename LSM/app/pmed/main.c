...
int main(int argc, char** argv) {
    ...
    double exec_time[40];
    int opt_values[40] = {127, 98, 93, 74, 48, 84, 64, 55, 37, 20, 59, 51, 35, 26, 18, 47, 39, 28, 18, 13, 40, 38, 22, 15, 11, 38, 32, 18, 13, 9, 30, 29, 15, 11, 30, 27, 15, 29, 23, 13};
    // To store mid_values
    ...
    for(int instance=1;instance<2;instance++){
        ...
        if(strcmp(algorithm, "Gon+") == 0){
            ...
            for(int p=0;p<n;p++){
                ...
                for(int i=1;i<k;i++){
                    // UPDATE DISTANCE
                    for(int j=0;j<n;j++){
                        if(matrix[j][C_temp[i-1]] < distance[j]){
                            distance[j] = matrix[j][C_temp[i-1]];
                        }
                    }
                    // SEARCH FOR FARTHEST VERTEX
                    float max_dist = 0;
                    ...
                    C_temp[i] = farthest_vertex;
                }
                // UPDATE DISTANCE AGAIN
                for(int i=0;i<n;i++){
                    if(matrix[i][C_temp[k-1]] < distance[i]){
                        distance[i] = matrix[i][C_temp[k-1]];
                    }
                }
                max_dist = 0;
                for(int i=0;i<n;i++){
        ...
        if(strcmp(algorithm, "HS") == 0){
            ...
            while(high - low > 1){
                ...
                if(previouslyComputed(r, counter, mid_value)){
                    ...
                    float max_dist = 0;
                    for(int j=0;j<n;j++){
                        if(matrix[j][C_temp[k-1]] < distance[j]){
                            distance[j] = matrix[j][C_temp[k-1]];
                        }
                    }
                    for(int j=0;j<n;j++){
                        if(distance[j] > max_dist){
            ...
            while(high - low > 1){
                ...
                if(previouslyComputed(r, counter, mid_value)){
                    ...
                    for(int i=0;i<k;i++){
                        ...
                        C_temp[i] = max_score_neighbor;
                        // UPDATE SCORE
                        int s_size = 0;
                        for(int j=0;j<n;j++){
                            if(matrix[j][C_temp[i]] <= r && dominated[j] == false){
                                dominated[j] = true;
                                S[s_size] = j;
                                s_size++;
                            }
        ...
        if(ls == 1){
            ...
            float distance[n];
            float distance_copy[n];
            float best_size = +INFINITY;
            int critical_vertices[n];
            int assigned_center[n];
            int assigned_center_copy[n];
            int assigned_center_index[n];
            int assigned_center_index_copy[n];
            ...
            double time_spent = 0;
            int tries_count = 0;
            float current_solution_size;

            // Get the initial solution size ***************************************
            for(int i=0;i<n;i++){
                distance[i] = 0;
                distance_copy[i] = 0;
            }
            ...
            for(int i=0;i<n;i++){
                ...
                for(int j=0;j<k;j++){
                    if(matrix[i][C[j]] < min_dist){
                        ...
                        assigned_center[i] = C[j];
                        assigned_center_index[i] = j;
                        assigned_center_copy[i] = C[j];
                        assigned_center_index_copy[i] = j;
                    }
                }
                distance[i] = min_dist;
                distance_copy[i] = min_dist;
                if(max_dist < min_dist){
                    max_dist = min_dist;
                }
            }
            best_size = max_dist;
            current_solution_size = max_dist;
            printf("initial solution size: %f \n", current_solution_size);
            // **********************************************************************

            //while(time_spent < 90000){ //4200
            //while(tries_count < 10000){ //4200
            while(current_solution_size > opt_values[instance-1]){

                // Get the critical vertex O(n)*******************************************
                int critical_vertex = 0;
                for(int i=0;i<n;i++){
                    if(distance[i] == current_solution_size){
                        critical_vertex = i;
                        break;
                    }
                }
                ...
                // Get the critical vertices O(n)************************************
                int critical_vertices_cardinality = 0;
                for(int i=0;i<n;i++){
                    if(matrix[critical_vertex][i] < best_size){
                        critical_vertices[critical_vertices_cardinality] = i;
                        critical_vertices_cardinality++;
                    }
                }
                // ******************************************************************

                // Select a vertex at random from the critical vertices**************
                int r = rand() % critical_vertices_cardinality;
                int random_vertex = critical_vertices[r];
                // ******************************************************************

                // Change the assigned center with the random vertex*****************
                //C[assigned_center[critical_vertex]] = random_vertex;
                // ******************************************************************

                // Get the distance from every vertex to the new solution************
                for(int i=0;i<n;i++){
                    if(assigned_center[i] != assigned_center[critical_vertex]){
                        if(matrix[i][random_vertex] <= distance[i]){
                            distance_copy[i] = matrix[i][random_vertex];
                            assigned_center_copy[i] = random_vertex;
                            assigned_center_index_copy[i] = assigned_center_index[critical_vertex];
                        }
                    }else{
                        min_dist = +INFINITY;
                        int center = 0;
                        int center_index = 0;
                        for(int j=0;j<k;j++){
                            if(C[j] != assigned_center[critical_vertex]){
                                if(matrix[i][C[j]] < min_dist){
                                    min_dist = matrix[i][C[j]];
                                    center = C[j];
                                    center_index = j;
                        ...
                        if(matrix[i][random_vertex] <= min_dist){
                            distance_copy[i] = matrix[i][random_vertex];
                            assigned_center_copy[i] = random_vertex;
                            assigned_center_index_copy[i] = assigned_center_index[critical_vertex];
                        }else{
                            distance_copy[i] = min_dist;
                            assigned_center_copy[i] = center;
                            assigned_center_index_copy[i] = center_index;
                        }
                    }
                }
                // ******************************************************************

                // Get new solution size ********************************************
                max_dist = 0;
                for(int i=0;i<n;i++){
                    if(distance_copy[i] > max_dist){
                        max_dist = distance_copy[i];
                    }
                }
                // ******************************************************************

                // Is this a better solution? ********************************************
                if(max_dist < current_solution_size){
                    current_solution_size = max_dist;
                    memcpy(distance, distance_copy, sizeof(distance));
                    memcpy(assigned_center, assigned_center_copy, sizeof(assigned_center));
                    memcpy(assigned_center_index, assigned_center_index_copy, sizeof(assigned_center_index));
                    C[assigned_center_index[critical_vertex]] = random_vertex;
                }else{
                    float r = (rand() % 1000) / 1000.0;
                    float T = 0.1;
                    if(r <= exp((-(max_dist - current_solution_size))/T)){
                        current_solution_size = max_dist;
                        memcpy(distance, distance_copy, sizeof(distance));
                        memcpy(assigned_center, assigned_center_copy, sizeof(assigned_center));
                        memcpy(assigned_center_index, assigned_center_index_copy, sizeof(assigned_center_index));
                        ...
                        // do nothing
                        memcpy(distance_copy, distance, sizeof(distance));
                        memcpy(assigned_center_copy, assigned_center, sizeof(assigned_center));
                        memcpy(assigned_center_index_copy, assigned_center_index, sizeof(assigned_center_index));
                    }
                }
                // ******************************************************************

                if(current_solution_size < best_size){
                    best_size = current_solution_size;
                }
